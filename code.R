...
#' neg_log_like
#'
#' compute the negative log-likelihood for a given set of parameters,
#' data, and model specification. 
#' 
#' @param beta These parameters are used to define the mean and standard deviation
#' of the normal distribution that models the relationship between the predictor(s) 
#' and the response variable.
#' @param data data frame containing the observed data, including CAD_Weight and Actual Weight
#' @param model A character string that specifies the model to be used,including "A" and "B",
...
neg_log_like <- function(beta, data, model) {
  ...
  if (model == "A") {
    mu <- beta[1] + beta[2] * x
    sigma <- exp(beta[3] + beta[4] * x)
  } else if (model == "B") {
    mu <- beta[1] + beta[2] * x
    sigma <- exp(beta[3]) + exp(beta[4]) * x^2
  }
  
  neg_log_likelihood <- -sum(dnorm(y, mean = mu, sd = sqrt(sigma), log = TRUE))
  return(neg_log_likelihood)
}

#' filament1_estimate
#' 
#'  estimate the parameters of two specified models ("A" or "B") 
#'  by minimizing the negative log-likelihood of the data given the model.
#' 
#' @param data containing the observed values, including
#' the variables used by the neg_log_like function, which are CAD_Weight
#' and Actual_Weight.
#' @param model A character string that specifies the model to be used,
#' including "A" and "B", each model with a different initial value for optimization.

filament1_estimate <- function(data, model) {
  if (model == "A") {
    init_values <- c(-0.1, 1.07, -2, 0.05)
  } else if (model == "B") {
    init_values <- c(-0.15, 1.07, -13.5, -6.5)
  }
  
  optim_res <- optim(par = init_values, fn = neg_log_like, data = data, model = model, hessian = TRUE, method = "BFGS")
  
  return(list(estimate = optim_res$par, hessian = optim_res$hessian))
}

#' compute_CI
#' 
#' It leverages the Hessian matrix obtained during optimization to 
#' compute the standard errors of the parameter estimates, which are then 
#' used to construct the 90% confidence intervals for model estimator returned
#' by filament1_estimate function
#' 
#' @param fit A list containing at least two components, which are estimate and 
#' hessian. Estimate is the vector of parameter estimates obtained from the optimization.
#' Hessian is the hessian matrix at the optimal parameters.

compute_CI <- function(fit) {
  ...
  lower_bound <- fit$estimate - z_value * se
  upper_bound <- fit$estimate + z_value * se
  
  return(data.frame(Estimate = fit$estimate, Lower = lower_bound, Upper = upper_bound, Std.Error = se))
}

#' log_prior_density
#'
#' calculate the logarithm of the joint prior density of the parameters theta.
#' @param theta a numeric vector representing the parameters theta_1 to theta_4
#' @param params another numeric vector representing the gamma parameters from
#' gamma_1 to gamma_4
...
#' @param mu The mean vector of the multivariate normal distribution used as the importance distribution.
#' @param S he covariance matrix of the multivariate normal distribution used as the importance distribution.

do_importance <- function(N, mu, S, ...) {
  # Sample from the multivariate normal distribution
  samples <- mvtnorm::rmvnorm(N, mean = mu, sigma = S)
  
  # Calculate the log-likelihoods of these samples using dmvnorm
  log_densities <- mvtnorm::dmvnorm(samples, mean = mu, sigma = S, log = TRUE)
  
  # Calculate log_weights (importance weights)
  log_weights <- -log_densities 
  
  # Normalize log_weights
  max_log_weight <- max(log_weights)
  log_weights <- log_weights - (max_log_weight + log(sum(exp(log_weights - max_log_weight))))
  
  # Create the data.frame
  results <- data.frame(beta1 = samples[,1], beta2 = samples[,2], 
                        beta3 = samples[,3], beta4 = samples[,4], 
...
#' make_CI
#' 
#' calculates a weighted credible interval for a set of data points x using the 
#' corresponding weights and the specified probability for the coverage of the interval.
#' 
#' @param x A numeric vector of data points for which the credible interval is to be calculated.
#' @param weights A numeric vector of weights associated with the data points in x
#' @param prob The probability coverage for the credible interval
